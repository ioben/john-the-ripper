...
# Apply custom patch to downloaded patch glibc-2.14-crypt.patch to support glibc 2.17
patch < /vagrant/contrib/glibc-crypto.patch

# Add new glibc-2.17-crypt.patch to glibc RPM's source dir
cp glibc-2.14-crypt.diff ../../SOURCES/glibc-2.17-crypt-bcrypt.patch

# Patch glibc SPEC file to use patch and include necessary files
cd ..
(cd ../SPECS; patch < /vagrant/contrib/glibc.spec.patch)
